...
do
    ...
    do
        ...
        # performance 벤치마크 실행
        ./run_and_time.sh $FRAMEWORK $BENCHMARK $MODEL $DEVICE --scenario $scenario $OPT
        # 벤치마크 결과 제출디렉토리 생성
    ...
    echo '==================== Accuracy ======================='
    # Accuracy 벤치마크 실행
    ./run_and_time.sh $FRAMEWORK $BENCHMARK $MODEL $DEVICE --scenario $scenario $OPT --accuracy
    
    # accuracy.txt 생성
    python3 $WORK_DIR/tools/accuracy-dlrm.py --mlperf-accuracy-file $WORK_DIR/output/$FRAMEWORK-$DEVICE/$BENCHMARK/mlperf_log_accuracy.json --imagenet-val-file $DATA_DIR/val_map.txt > $WORK_DIR/output/$FRAMEWORK-$DEVICE/$BENCHMARK/accuracy.txt
